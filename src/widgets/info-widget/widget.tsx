...
import { refreshIcon, caretRightIcon, caretDownIcon } from '@jupyterlab/ui-components';
import { ReactWidget, UseSignal, ToolbarButtonComponent, showDialog, Dialog } from '@jupyterlab/apputils';
import { SummaryTable, MorphometryTable, AdditionalInfoTable } from './summary_table';
...
namespace Components {
    ...
    render(): React.ReactNode {
      return (
        <>
          ...
          <div className={CONTAINER_CLASS + (this.state.visible ? '' : ' hidden')}>{this.props.children}</div>
        </>
      );
    }
  }
  ...
  export class ConnTableToolbar extends React.Component<{
    ...
    addAllNeurons(): void {
      ...
      const inworkspace_ids = [...filtered_ids].filter(ids => this.props.neu3d.isInWorkspace(ids.rid));
      const change_ids = [...filtered_ids].filter(ids => !inworkspace_ids.includes(ids));
    ...
    addAllSynapses(): void {
      ...
      const inworkspace_ids = [...filtered_ids].filter(ids => this.props.neu3d.isInWorkspace(ids.syn_rid));
      const change_ids = [...filtered_ids].filter(ids => !inworkspace_ids.includes(ids));
