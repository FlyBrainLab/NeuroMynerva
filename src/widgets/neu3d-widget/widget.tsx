...
namespace Private {
  ...
  function colorTextInsideQuotes(text: string, color: string): JSX.Element {
    ...
    while ((match = quotedTextRegex.exec(text)) !== null) {
      ...
      parts.push(<b><span style={{ color }}>{group}</span></b>);
      lastIndex = endIndex;
    }
  ...
  export function createFooterBar(neu3d: Neu3DWidget): HTMLDivElement {
    ...
    hintButton.onclick = () => {
      ...
      const hint_ul = hints.map((h, idx) => (
        <li key={idx}>
          <b><span style={TextStyle1}>{h.query}</span></b>
          {`: ${h.effect} Examples:`}
          <ul>
      ...
      if (hint_ul.length > 0) {
        hint_header = <p>Next, we explain the rules for defining the criterion of the query, using the verb show as an example.</p>;
      }
      showDialog({
        ...
        body: (
          <>
            ...
            <ul>Your queries should start with a verb; the verbs supported right now are:
              <li><b><span style={TextStyle1}>show</span></b>: clear workspace and then show the queried neurons,</li>
              <li><b><span style={TextStyle1}>add</span></b>: add to the workspace the neurons queried,</li>
              <li><b><span style={TextStyle1}>remove</span></b>: remove from the workspace the queried neuron,</li>
              <li><b><span style={TextStyle1}>keep</span></b>: keep in the workspace only the neurons that meet the
                      criterion of the query,</li>
              <li><b><span style={TextStyle1}>hide</span></b>: hide the neurons that meet the criterion of the query
                      (this does not remove them from workspace, but reduce their visibility),</li>
              <li><b><span style={TextStyle1}>pin</span></b>: pin the neurons that meet the criterion of the query.
                      Pinned neurons are automatically highlighted, and cannot be removed by the "trash can" button
                      on top of the NeuroNLP window.</li>
              <li><b><span style={TextStyle1}>unpin</span></b>: unpin the neurons that meet the criterion of the
                      query,</li>
              <li><b><span style={TextStyle1}>color</span></b>: color the neurons that meet the criterion of
                      the query with a user defined color (can be hex color code, e.g., FF0000 for red), or these
                      predefined colors.</li>
            </ul>
            <ul>Additionally, the following single word commands are supported:
              <li><b><span style={TextStyle1}>clear</span></b>: clear up the workspace, removing all neurons
                      and synapses,</li>
              <li><b><span style={TextStyle1}>undo</span></b>: undo your last query.</li>
            </ul>
            {hint_header}
            <ul> {hint_ul}</ul>
          </>
