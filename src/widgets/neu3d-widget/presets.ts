...
export const PRESETS: { [name in PRESETS_NAMES]: PresetEntry } = {
  'larva(l1em)': {
    ...
    hints: [
      ...
      {
        query: 'add /r(.*)OSN-(.*)/r',
        effect: 'add to the current workspace any neuron whose name matches the regular expression.'
      },
      {
        query: 'show $5813014882$',
        effect: 'show a single neuron corresponding to the Hemibrain BodyID 5813014882.'
      },
      {
      ...
      {
        query: 'add postsynaptic gabaergic $MBON$ neurons with at least 10 synapses',
        effect:
  ...
  'adult(hemibrain)': {
    ...
    hints: [
      ...
      },
      {
        ...
        effect: 'show neurons that have input site from EB and has output sites in PB.'
      }
    ]
  },
  default: {
