...
export class Neu3DModelTable {
  ...
  removeAllMeshes(active = true): void {
    const orids: string[] = this.meshTabulator.getData(active ? 'active' : '').map((r: any) => r.orid);
    this.neu3d.removeByRid(orids);
  }
  ...
  hideAllNeurons(active = true): void {
    const rids: string[] = this.neuronTabulator.getData(active ? 'active' : '').map((r: any) => r.rid);
  ...
  hideAllMeshes(active = true): void {
    const rids: string[] = this.meshTabulator.getData(active ? 'active' : '').map((r: any) => r.rid);
    this.neu3d.neu3d.hide(rids);
  }
  ...
  readonly neuronColumns = [
    ...
    },
    {
      ...
      headerSort: false,
      width: 32,
      formatter: (cell: unknown, formatterParams: unknown) => {
