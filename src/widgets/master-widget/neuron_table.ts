...
export class Neu3DModelTable {
  constructor(props: {
    ...
    const { neurons, neuropils } = this.parseData(this.neu3d);
    ...
    this.neu3d.model.dataChanged.connect((caller, change) => {
      ...
      const neuropilIdx = this.neuropils.findIndex(row => row.rid === rid);
   ...
   */
  removeAllNeurons(active?: boolean): void {
    const unames: string[] = this.neuronTabulator
   ...
   * @param active if true, only remove active neurons in the tabulator
   */
  removeAllNeuropils(active?: boolean): void {
    const unames: string[] = this.neuropilTabulator
      .getData(active ? 'active' : '')
      .map((r: any) => r.uname);
    // this.neu3d.removeByUname(unames);
  ...
  parseData(
    neu3d: Neu3DWidget
  ): { neurons: Array<any>; neuropils: Array<any> } {
    const neurons: Array<any> = [];
    const neuropils: Array<any> = [];
