...
async function activateFBL(
  ...
  Promise.all([app.restored, settings.load(extension.id)])
    ...
    .catch(reason => {
      console.error(`Something went wrong when reading the FFBO Processor Settings.\n${reason}`);
    });
...
export namespace FBL {
  export function getProcessors(setting?: ISettingRegistry.ISettings): FFBOProcessor.IProcessors {
    ...
    return FFBOProcessor.arrToDict(
      (setting.get('fbl-processors').composite as any) as FFBOProcessor.ISettings[]
    );
  }
  ...
  export async function createFBLWidget(options: {
    ...
    const { app, Module, icon, moduleArgs, fbltracker, ModuleName, status, add_widget_options } = options;

    const sessionContext =
      moduleArgs.sessionContext ?? fbltracker.trackers[ModuleName].currentWidget?.content?.sessionContext;
